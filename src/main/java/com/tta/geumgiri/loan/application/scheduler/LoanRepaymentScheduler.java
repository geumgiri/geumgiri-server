        ...
        for (Loan loan : loans) {
            if (!loan.isPaid()) {
                if (loan.getInstallments() <= 0){
                    System.out.println("[대출] 상환 횟수가 0인 대출은 처리할 수 없습니다.");
                    continue;
                }
                try {
                    ...
                    if (loan.isPaid()) {
                        messageBuilder.append("[대출] 사용자 ").append(loan.getMember().getName()).append("이(가) 대출한 대출(").append(loan.getId()).append(")의 상환이 완료되었습니다: ").append(loan.getPaidInstallments()).append("/").append(loan.getInstallments()).append("회 상환 완료되었습니다.\n");
                    } else {
                        messageBuilder.append("[대출] 사용자 ").append(loan.getMember().getName()).append("이(가) 대출한 대출(").append(loan.getId()).append(")의 일부가 상환되었습니다: ").append(loan.getPaidInstallments()).append("/").append(loan.getInstallments()).append("회차가 상환되었습니다.\n");
                    }
                } catch (IllegalArgumentException e) {
                    System.out.println("[대출] 잔액 부족으로 대출 상환 실패: " + e.getMessage());
                }
            }
